...
public class Question implements Serializable, Cloneable {
	...
	public String print(String showWork,String studentAnswer,Integer attemptsRemaining,boolean scoreWork) {
		...
		case 5: // Numeric Answer
			...
			buf.append("<div id=showWork" + this.id + " style='display:none'>"
					...
					+ "maxlength=500 placeholder='Show your work here. " + (scoreWork?"You must show clear evidence of sound thinking to receive full credit for a correct answer.":"This is optional scratch space that will be visible to your instructor.") + "' "
	...
	String printAllToStudents(String studentAnswer,boolean showDetails,boolean reportable,String showWork) {
		...
		if (studentAnswer==null || studentAnswer.isEmpty()) buf.append("<b>No answer was submitted for this question item.</b><p></p>");
			...
			case 5: // Numeric Answer
				...
				if (correctValue && correctSigFigs && correctWork) buf.append("&nbsp;<IMG SRC=/images/checkmark.png ALT='Check mark' align=bottom>");
						...
						buf.append("<IMG SRC=/images/partCredit.png ALT='wrong number of sig figs' align=middle>"
		...
		if (reportable) {
			...
			buf.append("<INPUT TYPE=BUTTON VALUE='Submit Feedback' "
					+ "onClick=\" return ajaxSubmit('/Feedback?UserRequest=ReportAProblem','" + this.id + "','" 
					+ Arrays.toString(this.parameters) + "','" + studentAnswer + "',"
					+ "encodeURIComponent(document.getElementById('suggest" + this.id + "').Notes.value),"
					+ "encodeURIComponent(document.getElementById('suggest" + this.id + "').Email.value)); return false;\" />"
					+ "</div></FORM><br/>");
			buf.append("</div>");
		}
		return buf.toString(); 
	...
	String renderChemicalStructureComposer(String fieldName, String molfile, boolean readOnly, boolean showMolfileDataPanel) {
		...
		boolean deferredOpen = !readOnly && !showMolfileDataPanel;
		String bridgeNonce = Long.toHexString(Double.doubleToLongBits(Math.random()));
		buf.append("<div style='margin:0.75rem 0;'>");
		if (deferredOpen) appendDeferredOpenComposerMarkup(buf, drawModeId, drawPanelId, smilesPanelId, smilesId, loadId, frameId, bridgeNonce);
		else appendImmediateComposerMarkup(buf, readOnly, smilesId, frameId, bridgeNonce);
		buf.append("<span id='" + statusId + "' style='display:none' aria-hidden='true'></span>");
		appendMolfileStorageMarkup(buf, showMolfileDataPanel, detailsId, textareaId, fieldName, readOnly, safeMolfile);
		if (!readOnly) buf.append(buildChemicalStructureComposerScript(frameId, textareaId, statusId, smilesId, drawModeId, loadId, drawPanelId, smilesPanelId, deferredOpen));
		buf.append("</div>");
		return buf.toString();
	}

	void appendDeferredOpenComposerMarkup(StringBuffer buf, String drawModeId, String drawPanelId, String smilesPanelId, String smilesId, String loadId, String frameId, String bridgeNonce) {
	...
	void appendImmediateComposerMarkup(StringBuffer buf, boolean readOnly, String smilesId, String frameId, String bridgeNonce) {
		if (!readOnly) {
	...
	void appendMolfileStorageMarkup(StringBuffer buf, boolean showMolfileDataPanel, String detailsId, String textareaId, String fieldName, boolean readOnly, String safeMolfile) {
	...
	String buildChemicalStructureComposerScript(String frameId, String textareaId, String statusId, String smilesId, String drawModeId, String loadId, String drawPanelId, String smilesPanelId, boolean deferredOpen) {
		return "<script>(function(){"
				...
				+ "function showTool(){if(!deferredOpen)return;if(drawPanel)drawPanel.style.display='';if(smilesPanel)smilesPanel.style.display='';ensureEditorLoaded();}"
				...
				+ "function notifyHostFormAnswerChanged(){if(!hostForm)return;hostForm.setAttribute('data-chem-interacted','true');hostForm.dispatchEvent(new Event('input',{bubbles:true}));hostForm.dispatchEvent(new Event('change',{bubbles:true}));}"
				...
				+ "if(data.type==='setMolfileResult'){const requestId=String(data.requestId||'');const isPreloadReply=expectedPreloadRequestId&&(!requestId||requestId===expectedPreloadRequestId);if(isPreloadReply){expectedPreloadRequestId='';const loaded=data.molfile||'';if(!isEmptyTemplate(loaded)){field.value=loaded;allowTemplateOverwrite=false;notifyHostFormAnswerChanged();updateStatus('Structure synchronized.','#0a6');}else{updateStatus('Saved structure load could not be confirmed.','#B20000');}return;}updateStatus(field.value?'Saved structure loaded.':'Editor cleared.','#0a6');return;}"
				+ "if(data.type==='setStructureResult'){const requestId=String(data.requestId||'');if(expectedStructureRequestId&&requestId&&requestId!==expectedStructureRequestId)return;expectedStructureRequestId='';const loaded=data.molfile||'';if(!isEmptyTemplate(loaded)){field.value=loaded;allowTemplateOverwrite=false;pendingStructureText='';notifyHostFormAnswerChanged();updateStatus('Structure loaded from SMILES.','#0a6');if(pendingSubmit&&!expectedSyncRequestId){updateStatus('Syncing structure before submit...','#555');requestSyncForSubmit();}}else{updateStatus('Unable to load the SMILES string into the editor.','#B20000');if(pendingSubmit)finalizeSubmit();}return;}"
				+ "if(data.type==='structureChanged'){const incoming=data.molfile||'';const incomingEmpty=isEmptyTemplate(incoming);field.value=incoming;if(!incomingEmpty){allowTemplateOverwrite=false;notifyHostFormAnswerChanged();}else if(hostForm){hostForm.dispatchEvent(new Event('input',{bubbles:true}));hostForm.dispatchEvent(new Event('change',{bubbles:true}));}return;}"
				+ "if(data.type==='molfile'){const requestId=String(data.requestId||'');if(!expectedSyncRequestId)return;if(requestId&&requestId!==expectedSyncRequestId)return;expectedSyncRequestId='';const incoming=data.molfile||'';const incomingEmpty=isEmptyTemplate(incoming);field.value=incoming;updateStatus(field.value&&!incomingEmpty?'Structure synchronized.':'Editor is empty.','#0a6');if(!incomingEmpty){allowTemplateOverwrite=false;notifyHostFormAnswerChanged();}else if(hostForm){hostForm.dispatchEvent(new Event('input',{bubbles:true}));hostForm.dispatchEvent(new Event('change',{bubbles:true}));}if(pendingSubmit)finalizeSubmit();return;}"
